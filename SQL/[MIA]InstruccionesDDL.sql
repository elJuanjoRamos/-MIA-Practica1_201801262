...
delimiter //
CREATE PROCEDURE CALL_CONSULTA5()
BEGIN
	SELECT P.id_person, P.name_person, MONTH(P.date_registro) AS mes_registro, sum(total) as total 
    FROM VENTA 
    JOIN 
...
delimiter //
CREATE PROCEDURE CALL_CONSULTA7()
BEGIN
	

	SELECT PE.id_person , PE.name_person, PE.mail_person, PE.phone_person, PE.date_registro, concat(PE.direction, ",", PE.city) as direction, sum(quantity) as total_vendido,  sum(total) as total_dinero FROM DETAILCOMPRA
		JOIN COMPRA AS CO ON CO.id_compra = DETAILCOMPRA.id_compra
		JOIN PRODUCT AS P ON P.id_product = DETAILCOMPRA.id_product
		JOIN PRODUCT_CATEGORY as C on C.id_category = P.id_category
		JOIN PERSON AS PE ON CO.id_person = PE.id_person
	WHERE C.name_category = 'Fresh Vegetables'
		group by CO.id_person order by total_dinero desc LIMIT 5;

END; //


delimiter //
CREATE PROCEDURE CALL_CONSULTA8()
BEGIN

SELECT P.id_person, P.direction, P.city, P.postal_code, P.region, sum(total) as total 
FROM VENTA 
JOIN 
	PERSON AS P ON P.id_person = VENTA.id_person 
group by
...
delimiter //
CREATE PROCEDURE CALL_CONSULTA9()
BEGIN

select P.name_person, P.phone_person, DT.id_compra, sum(quantity) as cant_comprada, sum(total) as total 
from DETAILCOMPRA AS DT
JOIN 
...
delimiter //
CREATE PROCEDURE CALL_CONSULTA10()
BEGIN


SELECT P.id_person, P.name_person, P.mail_person, P.phone_person, P.date_registro,
CONCAT(P.direction, " -- " ,P.region, " -- ", P.city) AS ADRESS,  sum(quantity) as cantidad_comprada FROM VENTA
JOIN 
	PERSON AS P ON P.id_person = VENTA.id_person
JOIN 
